...
class VRAuthCreationForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        try:
            VRAuth._default_manager.get(username=username)
    ...
    def clean_email(self):
        ...
        # 判断是这个email 用户是否存在
        try:
            VRAuth._default_manager.get(email=email)
        except VRAuth.DoesNotExist:
            return email
        raise forms.ValidationError(
    ...
    def save(self, commit=True):
        user = super(VRAuthCreationForm, self).save(commit=False)
...
class VRAuthPasswordRestForm(forms.Form):
